...
function buildFastSystemPrompt(): string {
  return [
    ...
    "```json",
    "[",
    "  { \"kind\": \"click\",  \"label\": \"<visible button text>\",      \"role\": \"button\",   \"hint\": \"open the form\" },",
    "  { \"kind\": \"type\",   \"label\": \"<input label / placeholder>\", \"value\": \"<text>\",  \"hint\": \"name the new item\" },",
    "  { \"kind\": \"press\",  \"key\":   \"Enter\",                          \"hint\": \"submit\" },",
    "  { \"kind\": \"wait\",   \"ms\":    1500,                              \"hint\": \"the result appears\" }",
    "]",
    "```",
    "",
    ...
    "",
    "Step kinds:",
    "- `click` — click a button / link / item. label + (optional) role.",
    "- `type` — focus an input, type into it. label + value.",
    "- `press` — keyboard key (Enter, Escape, Tab, ArrowDown). key.",
    "- `select` — pick an option from a <select>. label + value.",
    "- `wait` — pause so a state is visible. ms (default 1500). REQUIRED at end of demo.",
