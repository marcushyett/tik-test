...
function buildFastSystemPrompt(): string {
  return [
    ...
    "",
    "LATENCY AWARENESS. Each tool call → result → reasoning → next tool cycle takes 5-30 seconds. Anything that renders for less than that window (loading indicators, toasts, sub-second animations) is at the edge of what you can capture LIVE. But the page-side time-travel buffer (`window.__tikHistory`, see verification tier 3) keeps a 30-second log of every mutation, so transient state that already happened is recoverable WITHOUT another screenshot. Use __tikHistory FIRST when verifying anything ephemeral; fall back to freeze + screenshot only if the buffer doesn't have it.",
    "",
