...
function buildFastSystemPrompt(): string {
  return [
    ...
    "- PASS-2 STARTS WITH YOUR PASS-1 END STATE. Pass 2 reuses the same storage state (cookies, localStorage, IndexedDB, in-memory React state if persisted) you left at the end of validation. Anything you created, renamed, deleted, or toggled is STILL THAT WAY when the demo replays. Plan accordingly:",
    "  • Renaming demo: if you renamed 'Task A' → 'Task B' in pass 1, the list now contains 'Task B' (no 'Task A'). Don't write a STEPS rename of 'Task A' — pass 2 won't find it. Either (a) pick a DIFFERENT existing task to rename and choose a NEW unique name (e.g. rename 'Ship Taskpad v0.1' to 'Ship Taskpad v1.0 final'), or (b) start the demo with a fresh task you create as the first step.",
    "  • Toggle demo (pin / star / archive): if you already pinned a task in pass 1, that task is still pinned. Pick a different task that's NOT yet pinned, or start by un-pinning then re-pinning so the viewer sees the action take effect.",
    "  • Create / add demo: usually fine because each create produces a new entity, but make sure the new value is DIFFERENT from anything you created in pass 1 (e.g. don't add 'Buy milk' twice — use a fresh string).",
    "  • Delete demo: pick a task that still exists, not one you already deleted in pass 1.",
    "  • If the only safe path is to RESET the relevant data first, include the reset as the first step (a click on a 'Clear all' button, or a navigation that creates a fresh list). Reset must use real UI controls — don't reset via DOM or fetch.",
...
function buildMeticulousSystemPrompt(): string {
  return [
    ...
    "- PASS-2 STARTS WITH YOUR PASS-1 END STATE. Storage / cookies / persisted state from validation are still there. Anything you renamed, toggled, deleted, or created in pass 1 is still in that state. Plan around it: choose UNIQUE new values for renames (don't rename 'A' → 'B' if you already did that — pass 2 sees 'B' and your STEPS won't find 'A'), pick UN-toggled rows for toggle demos, or include a real-UI reset as the first step. NEVER reset via DOM or fetch.",
