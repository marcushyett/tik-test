...
function buildFastSystemPrompt(): string {
  return [
    ...
    "- PASS-2 STARTS WITH YOUR PASS-1 END STATE. Pass 2 reuses the same storage state (cookies, localStorage, IndexedDB, in-memory React state if persisted) you left at the end of validation. Anything you created, renamed, deleted, or toggled is STILL THAT WAY when the demo replays.",
    "- THE NUMBER ONE FAILURE MODE: emitting a `type` STEPS value that you already typed during pass 1. The field already contains that value, so pass 2 clicks the field, clears it, types the SAME thing again, presses Enter — the viewer sees the cursor move but the list doesn't visibly change. The demo silently lies about the feature working.",
    "- HARD RULE FOR `type` AND `select` VALUES IN STEPS: the value MUST be different from every value you typed/selected in pass 1, AND different from whatever is currently on screen. If you renamed 'Ship Taskpad v0.1' to 'Ship Taskpad v1.0 final' in pass 1, your STEPS rename target must be a DIFFERENT task (e.g. 'Review draft spec') and a DIFFERENT new name (e.g. 'Review draft spec — locked in'). Never re-type a name you already typed. Never pick a target whose existing text equals what you'd type.",
    "- HARD RULE FOR TOGGLES (pin / star / archive / favourite / complete): pick a row whose toggle is currently OFF. If you already pinned a task, the demo MUST pin a different one. Same for un-pinning — only un-pin a row that's currently pinned.",
    "- HARD RULE FOR DELETE: pick a row that still exists at pass-1 end. Don't delete the row you already deleted.",
    "- ADD / CREATE demos are usually safe because each create makes a new entity. Still, pick fresh values — don't add 'Buy milk' twice.",
    "- WORKED EXAMPLE — inline title edit on Taskpad: pass 1 added a task and renamed it 'Review final spec v2'. WRONG STEPS: rename the same task to 'Review final spec v2' again (no visible change). RIGHT STEPS: click a DIFFERENT task whose title is its original placeholder, rename it to a fresh string like 'Locked draft, ready to ship'. The list reflects a real change at pass-2 end.",
    "- If the only safe path is to RESET the relevant data first, include the reset as the first step (a click on 'Clear all' or similar). Reset must use real UI controls — never DOM or fetch.",
    ...
    "- THE DEMO MUST EXPLICITLY SHOW THE GOAL BEING COMPLETED. The final frame should literally show the user-visible signal that the goal succeeded — the toast, the new row at the top of the list, the changed badge text, the new title persisted, the empty state replacing the items. If your goal is 'pin a task to top', the last frame is the task sitting at position 1 with the pinned styling visible. If your goal is 'edit a title inline', the last frame is the list showing the NEW title, with the edit input gone. Don't end mid-action, don't end with a cursor still moving, don't end before the success state has had time to paint.",
    "- Use the closing `wait` to LET THE COMPLETION REGISTER. 1500-2500ms is enough for a viewer to read a toast or notice a row reorder. The `hint` on this wait is the narrator's success-confirmation cue — name the visible signal explicitly: \"the pinned task is at the top\", \"the toast confirms the save\", \"the list reflects the new title\".",
...
function buildMeticulousSystemPrompt(): string {
  return [
    ...
    "- PASS-2 STARTS WITH YOUR PASS-1 END STATE. Storage / cookies / persisted state from validation are still there.",
    "- HARD RULE for `type`/`select` STEPS: the value MUST differ from every value you typed/selected in pass 1, AND must differ from what's currently on screen. Re-typing the SAME value the field already contains makes pass 2 a silent no-op — the cursor moves but the list doesn't change. Worked example: if pass 1 renamed 'Ship v0.1' → 'Ship v1.0', the STEPS rename target must be a DIFFERENT task and a DIFFERENT new name.",
    "- HARD RULE for TOGGLES: pick a row whose toggle is currently OFF (pinned tasks already pinned can't be re-pinned visibly). HARD RULE for DELETE: pick a row that still exists. CREATE: pick fresh values not used in pass 1. NEVER reset via DOM or fetch — only real UI controls.",
    "- ALWAYS end with a `wait` step so the result of the last action sits on screen with the narrator describing it.",
    "- THE DEMO MUST EXPLICITLY SHOW THE GOAL BEING COMPLETED. The final frame is the user-visible success signal — toast, row at top, new title, empty state, etc. Don't end mid-action or before the success state paints. The closing wait's `hint` names the visible signal so the narrator can confirm it (\"the pinned task is at the top\", \"the toast confirms the save\").",
