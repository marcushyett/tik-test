...
function buildSystemPrompt(): string {
  return [
    ...
    "",
    "TRANSIENT UI STATES (loading spinners, toasts, skeletons that vanish in < 1s). Screenshots have ~200-1500ms of round-trip latency, so a spinner that appears for 200ms is GONE before your screenshot lands. DO NOT loop trying to catch it — the page has a built-in flight recorder that captures every transient state at sub-ms precision in the page's own clock. Use it:",
    "",
    "  STEP 1 — drop a marker BEFORE the action that triggers the transient:",
    "    browser_evaluate(`window.__tikFlight.mark('before-click')`)",
    "  STEP 2 — fire the action (browser_click, browser_type, etc).",
    "  STEP 3 — drop a second marker AFTER:",
    "    browser_evaluate(`window.__tikFlight.mark('after-click')`)",
    "  STEP 4 — read everything that happened between the markers:",
    "    browser_evaluate(`JSON.stringify(window.__tikFlight.between('before-click','after-click'))`)",
    "",
    "  The recorder captures: every loading-class element added/removed (.animate-pulse, .skeleton, .spinner, [aria-busy], [role=status], [role=progressbar], [data-state=loading]), every fetch/XHR start+end with URL+status+duration, every CSS animation that ran. The result includes a `summary` object with `loadingFlash: { firstAtMs, lastAtMs, durationMs }` so you can answer 'did a spinner show?' with one read instead of N screenshots. This is TIER-1 verification for transient UI — your OUTCOME can quote the recorder's evidence directly: 'OUTCOME: success — spinner with .animate-pulse appeared 12ms after click, hidden 340ms later (flight recorder)'.",
    "",
    "  You can also call window.__tikFlight.snapshot() at ANY time to read what's currently loading without needing a marker pair — useful right after clicking 'submit' to confirm a spinner is on screen RIGHT NOW.",
    "",
